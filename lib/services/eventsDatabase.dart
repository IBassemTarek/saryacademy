...
class EventDataBaseServices {
  final CollectionReference eventCard =
      FirebaseFirestore.instance.collection('EventCard');

  Future deleteeventCard() async {
    eventCard.doc(uid).delete();
    var snapshots =
  ...
  void initeventCard(
      {String imageURL, String text, Timestamp date, String title}) async {
    final DocumentReference documentReference =
        eventCard.doc(uid).collection("ListOfEventCard").doc();
  ...
  Stream<List<EventCard>> get eventCardsData {
    return eventCard
        .doc(uid)
        .collection("ListOfEventCard")
        .snapshots()
        .map(_eventCardListSnapShot);
  }

  Future checkIfEmpty({BuildContext context}) async {
    final isEmpty = Provider.of<IsEmptyModel>(context);
    final snapshot =
        eventCard.doc(uid).collection("ListOfEventCard").snapshots();
  ...
  Future addNewEventrData(
      {Timestamp date, String text, String title, String imageURL}) async {
    return await eventCard
        .doc(uid)
        .collection("ListOfEventCard")
        .doc(title)
